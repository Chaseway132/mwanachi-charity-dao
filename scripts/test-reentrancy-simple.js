...
async function main() {
  ...
  console.log("\nInitial FundAllocation balance:", ethers.formatEther(initialFundBalance), "ETH");

  // Fund the FundAllocation contract if needed
  if (initialFundBalance < ethers.parseEther("1")) {
    console.log("Funding the FundAllocation contract with 2 ETH...");
  ...
  const attacker = await ReentrancyAttacker.deploy(addresses.FUND_ALLOCATION);
  ...
  // This is just for testing - in a real scenario, you'd use different accounts
  for (let i = 0; i < 2; i++) {
    try {
  ...
  try {
    ...
    // This will fail in a real scenario, but we're just testing
    try {
      await proposalManagement.signProposal(proposalId);
      ...
      // In a real test, you would use a second account
    }
  } catch (error) {
  ...
  console.log("\nFinal FundAllocation balance:", ethers.formatEther(finalFundBalance), "ETH");
  ...
  if (attackerBalance > proposalAmount) {
    ...
    console.log("\n✅ Reentrancy attack failed or was prevented. The contract appears to be secure.");
    console.log(`Expected: ${ethers.formatEther(proposalAmount)} ETH`);
    console.log(`Actual: ${ethers.formatEther(attackerBalance)} ETH`);
  }
